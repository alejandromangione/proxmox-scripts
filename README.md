...
```bash
bash -c "$(wget --no-cache -qO- https://raw.githubusercontent.com/alejandromangione/proxmox/main/create.sh)"
```
...
```sh
sh -c "$(wget --no-cache -qO- https://raw.githubusercontent.com/alejandromangione/proxmox/main/install.sh)" -s --app nginx-proxy-manager
```
